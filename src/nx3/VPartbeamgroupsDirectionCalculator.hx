	...
	public function calculateBeamgroupsDirections():Void
	{
		...
		else if (partbeamgroups.length == 2)
		{
			...
			else 
			{
				for (beamgroup in beamgroups0)
				{
					beamgroup.setCalculatedDirection(EDirectionTools.uadToUd(voice0.nvoice.direction));
				}

				for (beamgroup in beamgroups1)
				{
					beamgroup.setCalculatedDirection(EDirectionTools.uadToUd(voice1.nvoice.direction));
				}
			}
		}
		else

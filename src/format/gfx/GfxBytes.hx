...
class GfxBytes extends Gfx
{
   ...
   override public function lineStyle(style:LineStyle)
   {
	  ...
	  //var capsStyleIdx = Type.enumIndex(style.capsStyle.getIndex());
	  var capsStyleIdx = 0; // style.capsStyle.getIndex();
      buffer.writeByte(capsStyleIdx);
	  trace(capsStyleIdx);
	  //var jointStyleIdx = Type.enumIndex(style.capsStyle);
	  var jointStyleIdx = 0; // style.capsStyle.getIndex();
	  trace(jointStyleIdx);
      buffer.writeByte(jointStyleIdx);

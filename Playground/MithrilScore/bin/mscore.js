...
Main.__name__ = true;
Main.main = function() {
	var nodes = window.document.querySelectorAll(".nx-scoredata");
	var _g = 0;
...
var MScoreModel = function(script,scoreWidth,scaling) {
	if(scoreWidth == null) scoreWidth = 800.0;
	var _g = this;
	this.xml = script.innerHTML;
	this.nscore = nx3.xml.ScoreXML.fromXmlStr(this.xml);
	this.id = thx.core.UUID.create();
	this.scoreWidth = scoreWidth;
	this.scaling = scaling;
	var timer = new haxe.Timer(2000);
	timer.run = function() {
		_g.renderScore(scoreWidth,scaling);
	};
};
MScoreModel.__name__ = true;
MScoreModel.__interfaces__ = [mithril.Model];
MScoreModel.prototype = {
	renderScore: function(scoreWidth,scaling) {
		if(scoreWidth == null) scoreWidth = 800.0;
		var len = Math.ceil(Math.random() * 4);
		var nscore = new nx3.NScore([new nx3.NBar([new nx3.NPart([new nx3.NVoice((function($this) {
			var $r;
			var _g = [];
			{
				var _g1 = 0;
				while(_g1 < len) {
					var i = _g1++;
					_g.push(new nx3.NNote(null,[new nx3.NHead()]));
				}
			}
			$r = _g;
			return $r;
		}(this)))])])]);
		var target = new nx3.render.TargetSvgXml(this.id,scaling);
		var renderer = new nx3.render.Renderer(target,0,0);
		renderer.renderScore(new nx3.PScore(nscore),0,0,scoreWidth);
...
var MScore = function(el) {
	this.el = el;
	var script = el.querySelector("script");
	this.model = new MScoreModel(script);
};
MScore.__name__ = true;
MScore.__interfaces__ = [mithril.View];
MScore.prototype = {
	view: function() {
		return m.m(".nx-score",{ style : { position : "absolute", height : this.model.svgHeight + "px", width : this.model.svgWidth + "px"}},m.trust(this.model.svg));
	}
};
var MScoreCanvas = function(el) {
	MScore.call(this,el);
};
MScoreCanvas.__name__ = true;
MScoreCanvas.__super__ = MScore;
MScoreCanvas.prototype = $extend(MScore.prototype,{
	view: function() {
		return m.m("div",{ style : { height : this.model.svgHeight + "px", width : this.model.svgWidth + "px"}},[MScore.prototype.view.call(this),m.m("canvas.nx-canvas",{ style : { position : "absolute", height : this.model.svgHeight + "px", width : this.model.svgWidth + "px"}})]);
	}
});
var MScoreCanvasPlayModel = function() {
	this.height = 24;
};
MScoreCanvasPlayModel.__name__ = true;
MScoreCanvasPlayModel.__interfaces__ = [mithril.Model];
var MScoreCanvasPlay = function(el) {
	MScoreCanvas.call(this,el);
	this.playModel = new MScoreCanvasPlayModel();
};
MScoreCanvasPlay.__name__ = true;
MScoreCanvasPlay.__super__ = MScoreCanvas;
MScoreCanvasPlay.prototype = $extend(MScoreCanvas.prototype,{
	view: function() {
		return m.m(".nx-play-score",{ style : { height : this.model.svgHeight + this.playModel.height + "px", width : this.model.svgWidth + "px"}},[m.m(".nx-play-toolbar",{ style : { height : this.playModel.height + "px", width : this.model.svgWidth + "px"}},[m.m("button",{ onmousedown : function(e) {
			js.Lib.alert("Hello");
		}},"play")]),MScoreCanvas.prototype.view.call(this)]);
	}
});
var IMap = function() { };
IMap.__name__ = true;

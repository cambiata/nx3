...
Main.main = function() {
	...
	openfl.Lib.get_current().addChild(new Main());
};
...
Main.prototype = $extend(openfl.display.Sprite.prototype,{
	...
	,init: function() {
		...
		s.get_graphics().beginFill(16711680);
		s.get_graphics().drawCircle(10,10,10);
		this.addChild(s);
		var doc = haxe.xml.Parser.parse(Main.clefG);
		...
		openfl.Lib.get_freeSnap().append(svgAsset);
		var svg = new openfl.display.Svg(svgAsset["use"]());
		this.addChild(svg);
	}
	,added: function(e) {
		this.removeEventListener(openfl.events.Event.ADDED_TO_STAGE,$bind(this,this.added));
		this.get_stage().addEventListener(openfl.events.Event.RESIZE,$bind(this,this.resize));
		this.init();
	}
	,__class__: Main
});
...
var DefaultAssetLibrary = function() {
	...
	DefaultAssetLibrary.type.set("assets/rabbit.svg",value1);
};
...
DefaultAssetLibrary.prototype = $extend(openfl.AssetLibrary.prototype,{
	...
	,getBitmapData: function(id) {
		try {
			...
			haxe.Log.trace("Error: Bitmap not found - " + id + " - " + DefaultAssetLibrary.path.get(id),{ fileName : "DefaultAssetLibrary.hx", lineNumber : 177, className : "DefaultAssetLibrary", methodName : "getBitmapData"});
			return null;
		}
	}
	,getBytes: function(id) {
			...
			if(null != this.get_mask() && (null == snapMask || "none" == snapMask)) {
				if(null != this.get_mask().snap) {
					if(!this.get_mask().__isOnStage()) {
						haxe.Log.trace("add child",{ fileName : "Svg.hx", lineNumber : 80, className : "openfl.display.Svg", methodName : "__render"});

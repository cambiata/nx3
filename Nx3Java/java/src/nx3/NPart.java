...
public  class NPart extends haxe.lang.HxObject
{
	...
	public static   void __hx_ctor_nx3_NPart(nx3.NPart __temp_me74, haxe.root.Array<nx3.NVoice> voices, nx3.EPartType type, nx3.EClef clef, nx3.EDisplayALN clefDisplay, nx3.EKey key, nx3.EDisplayALN keyDisplay)
	{
		__temp_me74.nvoices = voices;
		{
			int _g = 0;
			while (( _g < voices.length ))
			{
				nx3.NVoice voice = voices.__get(_g);
				 ++ _g;
				voice.npart = __temp_me74;
			}
			
		}
		
		if (( __temp_me74.nvoices.length > 2 )) 
		{
		...
		if (( type == null )) 
		{
			__temp_me74.type = nx3.EPartType.Normal;
		}
		 else 
		{
		...
		__temp_me74.clef = clef;
		if (( clefDisplay == null )) 
		{
			__temp_me74.clefDisplay = nx3.EDisplayALN.Layout;
		}
		 else 
		{
			__temp_me74.clefDisplay = clefDisplay;
		}
		
		__temp_me74.key = key;
		if (( keyDisplay == null )) 
		{
			__temp_me74.keyDisplay = nx3.EDisplayALN.Layout;
		}
		 else 
		{
			__temp_me74.keyDisplay = keyDisplay;
		}
